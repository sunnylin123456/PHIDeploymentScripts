...
if ($null -eq $identityNumber) {
    if (-not [string]::IsNullOrEmpty($registerKey)) {
        try {
            $cmdPath = Get-CmdFilePath
            Write-InfoMsg "Registering $ProductName node..."
            $process = Start-Process $cmdPath "-r -k $registerKey" -Wait -PassThru -NoNewWindow
            if ($process.ExitCode -ne 0) {
                throw "Failed to register $ProductName. Exit code: $($process.ExitCode)"
            }
            Write-InfoMsg "Registration successful."
        } catch {
            Write-ErrorMsg "Registration failed: $_"
        }
    } else {
        Write-InfoMsg "Install complete. You must provide -registerKey to register the node automatically."
    }
}
